...
> **About the figures in this document.** Every figure is generated by the Wolfram
> Language code shown inline beside it. The terrain base uses Wolfram's
> `GeoElevationData` for the Durham area (the pipeline itself uses USGS 3DEP);
> most parameter/score surfaces and distributions use **synthetic or randomized
> inputs** (`SeedRandom`/`RandomReal`, visible in each code block) to illustrate
> the shape of GRIME's scoring functions — they are conceptual illustrations, not
> plots of pipeline output. Where a real, reproducible counterpart exists it is
> called out in the text (e.g. `scripts/robustness_report.py`).
...
> **Illustrative mock.** The histogram above is drawn from Beta distributions
> (`SeedRandom[77]` in the code block) to show the *expected shape* of a
> robustness distribution — it is not model output. The real, reproducible
> histogram comes from `python3 scripts/robustness_report.py` (n = 500 Dirichlet
