...
The system works in two modes. The **research pipeline** ingests real 10m elevation data from USGS 3DEP, runs a full computational hydrology pipeline to extract stream networks, then scores every candidate point using live data from federal APIs where available (USGS, EPA, Census; anything unfetchable falls back to documented constants that the per-run provenance block reports). The **interactive dashboard** lets anyone click on any of 89,518 places across 239 countries, fetches real waterway geometry from OpenStreetMap, generates and scores net placements client-side, and renders color-coded results on a Mapbox map in under 5 seconds.
...
Each candidate is evaluated on 27 parameters *designed* to pull from federal and state sources (USGS NWIS/3DEP/NHDPlus, EPA TRI/ECHO/SEMS, Census ACS, USGS PAD-US, NC OneMap). Coverage is honest and recorded per run: the frozen June 2026 Durham run had **11 of 27 parameters varying live** (EPA ECHO and USGS StreamStats were down; PAD-US/NBI/SEMS were not yet wired), the fix-pass-2 regeneration reached **24 of 27**, and the final Fix 3 regeneration (July 2026) produced 147 Durham sites with **25 of 27 parameters varying**. The two constants are truthful CSO zero and the documented unsupported municipal-litter fallback; Fix 3 also completed all 449 configured regional outputs with 9,903 sites. The scoring engine drops constant columns and renormalizes the surviving weights; every output geojson's `provenance` block and `/api/weights` record the actual live/fallback split for that run. The environmental-justice index is reconstructed from Census ACS demographics because EPA decommissioned EJSCREEN in 2025. Flow velocity is estimated using Manning's equation applied to DEM-derived slope and channel geometry:
...
**Reproducible by design.** Every data source we use is free (Census ACS needs a free, instant API key; the rest are keyless). The offline demo dataset regenerates deterministically with zero network; the live scored datasets are shipped as frozen artifacts because re-running them needs the network-bound pipeline, a Census key, and the one-time bulk downloads recorded in `cache/*/PROVENANCE.txt`.
