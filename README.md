...
8. Mapbox renders: stream lines (cyan) + candidate dots (color-coded by score); where the city overlaps the committed live run's coverage (Durham NC), the real pipeline sites are drawn as dark-ringed dots alongside the demo estimates
...
9. **The shipped Durham dataset is a frozen artifact.** Regenerating it requires live federal APIs, a Census key, and DEM downloads (see §8 live-vs-fallback for what varied in each run).
