...
def run_pipeline(bbox=ELLERBE_BBOX, resolution=10, threshold=500,
    ...
    # (fix-pass-2 Phase 5) The raw stream network is no longer dumped to
    # streams.geojson: it was a write-only intermediate — raw UTM metres,
    # empty properties, no reader anywhere (API, dashboards, scripts) — that
    # accumulated 600 KB per run dir. The drawn rivers come from OSM/NHD.
